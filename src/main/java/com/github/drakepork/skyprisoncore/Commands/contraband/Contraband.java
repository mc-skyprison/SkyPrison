...
public class Contraband implements CommandExecutor {
	...
	public boolean InvCheckCont(Player target) {
		for (int n = 0; n < target.getInventory().getSize(); n++) {
			...
			if (i != null) {
				for (Material cb : plugin.contraband()) {
					if (i.getType() == cb) {
						return true;
					}
				}
	...
	public void cbPunish(Player target, int tmremaining) {
		if (plugin.cbed.contains(target)) {
			if (tmremaining > 0) {
				...
				Bukkit.getServer().dispatchCommand(plugin.getServer().getConsoleSender(), "jail " + target.getName());
				...
				plugin.cbed.remove(target);
				plugin.cbedMap.get(target).sendMessage("[" + ChatColor.BLUE + "Contraband" + ChatColor.WHITE + "]: " + ChatColor.GOLD + target.getName() + ChatColor.LIGHT_PURPLE + " has gone to jail!");
				plugin.cbedMap.remove(target);
			}
		}
	}
