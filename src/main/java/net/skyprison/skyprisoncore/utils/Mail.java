...
public class Mail {
    ...
    public static List<String> getBoxesWithMail(UUID pUUID) {
        ...
        try(Connection conn = db.getConnection(); PreparedStatement ps = conn.prepareStatement("SELECT name FROM mail_boxes WHERE id IN " +
                ...
                "UNION SELECT name FROM mail_boxes WHERE id = -1 AND EXISTS (SELECT 1 FROM mails WHERE receiver_id = ? AND collected = 0 AND mailbox_id = -1)")) {
    ...
    public static int getMailBox(Block b) {
        try(Connection conn = db.getConnection(); PreparedStatement ps = conn.prepareStatement(
                "SELECT id FROM mail_boxes WHERE x = ? AND y = ? AND z = ? AND world = ? AND is_placed = 1 AND deleted = 0")) {
            ps.setInt(1, b.getX());
            ps.setInt(2, b.getY());
            ps.setInt(3, b.getZ());
